...
docker wait shop-service-e2e-test

# Get the test container's exit code
TEST_EXIT_CODE=$(docker inspect shop-service-e2e-test --format='{{.State.ExitCode}}')

# Save test output
echo "Saving test output..."
docker logs shop-service-e2e-test > test-results/e2e-test-output.log
